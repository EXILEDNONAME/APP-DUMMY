...
class CrudCommand extends Command
{
  ...
  */
  public function handle()
  {
    ...
    $viewPath = ($this->option('view-path')) ? 'pages\\backend\\__main\\' . trim($this->option('view-path')) . '\\' : 'pages\backend\__main';
    ...
    $this->call('crud:controller', ['name' => $controllerNamespace . $modelName . 'Controller', '--crud-name' => $name, '--model-name' => $modelName, '--model-namespace' => $modelNamespace, '--view-path' => $this->option('view-path'), '--route-group' => $routeGroup, '--pagination' => $perPage, '--fields' => $fields, '--validations' => $validations]);
    $this->call('crud:model', ['name' => $modelNamespace . $modelName, '--fillable' => $fillable, '--table' => $tableName, '--pk' => $primaryKey, '--relationships' => $relationships, '--soft-deletes' => $softDeletes, '--foreign-keys' => $foreignKeys, '--view-path' => $this->option('view-path')]);
    $this->call('crud:migration', ['name' => $migrationName, '--schema' => $migrationFields, '--pk' => $primaryKey, '--indexes' => $indexes, '--foreign-keys' => $foreignKeys, '--soft-deletes' => $softDeletes, '--view-path' => $this->option('view-path')]);
    ...
    if (\App::VERSION() >= '5.3') {
      ...
      $addedMenu = File::append($menuSidebar, "\n<div class=\"kt-menu-item {{ (request()->is('dashboard/" . $this->routeName . "*')) ? 'active' : '' }}\" data-kt-menu-item-toggle=\"accordion\" data-kt-menu-item-trigger=\"click\">
    <a href=\"/dashboard/" . $this->routeName . "\" class=\"kt-menu-link flex items-center grow cursor-pointer border border-transparent gap-[10px] ps-[10px] pe-[10px] py-[6px]\">
...
");
  ...
  */
  ...
  protected function addRoutes() {
    $name = $this->argument('name');
    $routeGroup = $this->option('route-group') ? "\\" . ucwords($this->option('route-group')) : '';
    $routeControllerName = Str::singular($name) . 'Controller';
    $RouteNameAs = preg_replace("/\//", ".", $this->routeName);

    return ["
Route::group([
    'as' => 'dashboard.main.$RouteNameAs.',
    'prefix' => 'dashboard/$this->routeName',
    'namespace' => 'App\Http\Controllers\Backend\__Main$routeGroup',
    'middleware' => ['auth', 'verified'],
], function(){
    Route::get('chart', '$routeControllerName@chart')->name('chart');
    ...
    Route::get('selected-active', '$routeControllerName@selected_active')->name('selected-active');
    Route::get('selected-inactive', '$routeControllerName@selected_inactive')->name('selected-inactive');
    Route::get('selected-delete', '$routeControllerName@selected_delete')->name('selected-delete');
    Route::get('selected-delete-permanent', '$routeControllerName@selected_delete_permanent')->name('selected-delete-permanent');
    Route::get('selected-restore', '$routeControllerName@selected_restore')->name('selected-restore');
